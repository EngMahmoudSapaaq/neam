...
$requests = mysqli_query($conn, "SELECT * FROM `requests` WHERE beneficiary_id = '$beneficiary_id'") or die('Query failed: ' . mysqli_error($conn));
        ...
        <div class="row">
          <div class="text-md-start text-center py-6">
            <h1 class="mb-4 fs-9 fw-bold text-center">
              <span class="text-warning">Donation</span> Requests
            </h1>
          </div>
          <div class="col-12 m-auto">
            <table class="dataTable table">
              <thead>
                ...
                <th class="align-middle">Donator Name</th>
                <th class="align-middle">Donation Section</th>
                <th class="align-middle">Donation Quantity</th> 
...
if (isset($requests) && mysqli_num_rows($requests) > 0) {
    while ($request = mysqli_fetch_assoc($requests)) {
        ...
        echo '<tr>
                  ...
                  <td class="align-middle">'.$donor['name'].'</td>
                  <td class="align-middle">'.$section['name'].'</td>
                  <td class="align-middle">'.$donate['quantity'].'</td>   
                  <td class="align-middle">Description : '.$donate['description'].'</td>';
        
        if ($status=='pending'){
            echo '<td class="align-middle"><span class="badge bg-warning">Pending</span></td> ';
        }elseif ($status=='accepted'){
            echo '<td class="align-middle"><span class="badge bg-success">Accepted</span></td> ';
                          } else {
                              echo '<td class="align-middle"><span class="badge bg-danger">Rejected</span></td> ';
                          }
                  echo '</tr>';
        ...
        echo ' <tr>
                  ...
                  <td class="align-middle">there no data</td>
                  <td class="align-middle"></td>
                  <td class="align-middle"></td>   
                 <td class="align-middle"></td>
                 <td class="align-middle"></td>
