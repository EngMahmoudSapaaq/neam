...
$receive_appointments = mysqli_query($conn, "SELECT * FROM `receive_appointments` ") or die('Query failed: ' . mysqli_error($conn));
if (isset($_GET['appointment_id'])) {
    $appointment_id = $_GET['appointment_id'];
    
         $requests = "UPDATE `receive_appointments` SET `donor_consent` = 'accepted' WHERE `id` = '$appointment_id'";
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 m-auto">
                            <table class="dataTable table">
                                ...
                                <th class="align-middle">Donor consent</th>
...
if (isset($receive_appointments) && mysqli_num_rows($receive_appointments) > 0) {
    while ($receive_appointment = mysqli_fetch_assoc($receive_appointments)) {
        ...
        if ($this_donor_id == $donor_id) {
            echo '<tr>
            <td class="align-middle">' . $donate['id'] . '</td>';

            // Format the receive date to 'dd/mm/yyyy'
            $formattedDate = date('d/m/Y', strtotime($receive_appointment['donate_receive_date']));
            ...
            echo '<td class="align-middle"><span class="badge bg-secondary">' . $receive_appointment['donor_consent'] . '</span></td>
