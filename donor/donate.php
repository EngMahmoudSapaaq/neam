...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_POST['section_type'])) {
        foreach ($_POST['section_type'] as $check) {
            $section_id = $check;
        }
    }


    $insert = mysqli_query($conn, "INSERT INTO `donates`(address,quantity, description,image,status,donor_id,section_id)"
            . " VALUES('$Address','$Quantity', '$Description', '$image', 'available', '$donor_id', '$section_id')") or die('query failed');
    if ($insert) {
        move_uploaded_file($image_tmp_name, $image_folder);
        $message1[] = 'A donation has been added successfully';
    } else {
                ...
                <div class="container">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-6 text-md-start text-center py-6">
                            <h1 class="mb-4 fs-9 fw-bold">Make <span class="text-warning">Donation</span></h1>
                            <?php
                            if (isset($message)) {
                            ...
                            <form method="post" enctype="multipart/form-data" >
                                <div name="section_type[]">
                                    <label for="image">Section <span class="text-warning-darker">*</span></label>
                                    <select name="section_type[]" id="user_type" class="form-control border border-dark">
                                        <?php
                                        if (isset($sections) && mysqli_num_rows($sections) > 0) {
                                            ...
                                            echo '<option disabled>there are no sections</option>';
                                        }
                                        ?>
                                ...
                                <div class="mt-3">
                                    <label for="image">Image <span class="text-warning-darker">*</span></label>
                                    <input type="file" name="image" id="image" class="form-control border border-dark" required>
                                </div>

                                <div class="mt-3">
                                    <label for="address">Description <span class="text-warning-darker">*</span></label>
                                    <textarea type="text" name="Description" id="address" class="form-control border border-dark" required rows="5"></textarea>
                                </div>

                                <div class="mt-3">
                                    <label for="qty">Quantity <span class="text-warning-darker">*</span></label>
                                    <input type="number" name="Quantity" id="qty" min="1" step="1" class="form-control border border-dark" value="1" required>
                                </div>

                                <div class="mt-3">
                                    <label for="address">Address <span class="text-warning-darker">*</span></label>
                                    <input type="text" name="Address" id="address" class="form-control border border-dark" required>
                                    <span class="text-muted">- Your address is added here. You can change it.</span>
                                </div>

                                <div class="text-center text-md-start mt-3">
                                    <button class="btn btn-warning me-3 btn-lg" type="submit" name="submit">
                                        Donate&nbsp;&nbsp;<i class="fa fa-arrow-right"></i>
                                    </button>
                                </div>
                            </form>
                        </div>
